...
export default function DashboardLayout({
  ...
  const handleSendWhatsApp = async () => {
    ...
    try {
      const token = localStorage.getItem('boeking_token')
      const restaurantName = user?.restaurantName || 'us'
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/dashboard/send-whatsapp`, {
        ...
        body: JSON.stringify({ whatsapp_number: waNumber, message: waMessage })
      })
  ...
  const defaultMessage = `Hi! This is ${user?.restaurantName || 'the restaurant'}. We received your call about a table reservation. Reply to this message and our booking assistant will help you complete your reservation instantly!`
          ...
          <div className="relative bg-[#111827] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-0.5">Send a message to start a booking via WhatsApp</p>
              </div>
              <button
                onClick={() => setShowWhatsApp(false)}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-[#0B0F14] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-green-500/50 transition resize-none text-sm"
                />
                <p className="text-gray-600 text-xs mt-1">
                  Once the customer replies, the AI booking assistant will take over automatically.
                </p>
              </div>
